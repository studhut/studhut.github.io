...
export class SceneReader {
    ...
    async getSceneFormat() {
        var header: number = Utils.parseInt8Array(await this.buffer.getChunk(0, 3));
    ...
    async parsePNTRs() {
        ...
        var num_pntrs: number = await this.buffer.getInt32();
        console.log("# of PNTRs:", num_pntrs);

        for (var i: number = 0; i < num_pntrs; i++) {
            var pos: number = this.buffer.position;
            console.log(`Buffer Pos[${i}]:`, Utils.toHex(pos));
            var pntr_offset: number = await this.buffer.getInt32();
            ...
            var offset: number = await this.buffer.getInt32();
            if (offset != 0) {
    ...
    async loadBlocks() {
        ...
        while (this.file.size - this.buffer.position > 0) {
            if (!(await this.loadBlockAtCurrentPos(false))) break;
        }
    }

    async loadBlockAtCurrentPos(load_game_scene: boolean) {
        var save_pntr: number = this.buffer.position;
        var block_id: number = await this.buffer.getInt32();
        var block_size: number = await this.buffer.getInt32();

        var block_name: string = Utils.blockIdToString(
            Utils.flipEndian(block_id)
        );
        console.log(block_name);

        if (block_size > 1e8) return false;
        if (block_size == 0) console.log("Read zero-size block");
        if ((block_id = this.NU20_INT)) return false;
