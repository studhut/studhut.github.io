...
export class SceneReader {
    ...
    async getChunk(start, end) {
        var chunk: ArrayBuffer = await this.file.slice(start, end + 1).arrayBuffer();
        var chunk_array: Uint8Array = new Uint8Array(chunk);

        return chunk_array;
    }

    async getSceneFormat() {
        var header: number = Hex.parseUint8Array(await this.getChunk(0, 3));
